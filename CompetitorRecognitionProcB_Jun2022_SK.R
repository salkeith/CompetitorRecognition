...
load("DataForCompetitorRecognition.RData")
...
colnames(hsprox.pairs2)[31] <- "pair.id"
hsprox.post.pairs2 <- cbind(hsprox.post,hsprox.post.pairs.id[,2])
colnames(hsprox.post.pairs2)[31] <- "pair.id"
...
plot(prox.phylo$phylo,prox.phylo$mean.proximity,xlab="phylogenetic distance",
     ylab="mean proximity",cex.lab=1.5)
lines(lowess(prox.phylo$phylo,prox.phylo$mean.proximity,f=2/3))

# linear regression
lm.prox.phylo <- lm(mean.proximity~phylo,data=prox.phylo)
# add in quadratic
...
Signal.Proportion <- function(cssig.data,hssig.data,bootstrap.n,title) {
   ...
   cssig.data <- cssig.data[,c(26,1,3,19)]
   hssig.data <- hssig.data[,c(26,31,2,19)]
...
# linear regression
lm.sig.phylo <- lm(sig.prop~phylo,data=sig.phylo)
# add in quadratic
sig.phylo$phylo2 <- sig.phylo$phylo^2
sig.phylo2 <- sig.phylo[order(sig.phylo$phylo2), ]
lm.sig.phylo.quad <- lm(sig.prop~phylo+phylo2,data=sig.phylo2)
# plot QQ plot etc to explore model fit
par(mfcol=c(4,2))
plot(lm.sig.phylo)
plot(lm.sig.phylo.quad)
# What is R-squared value for each?
summary(lm.sig.phylo)
summary(lm.sig.phylo.quad)
# Use AICc to see which model is best fit
AICc(lm.sig.phylo)       # LOWEST AICc
AICc(lm.sig.phylo.quad) 
...
boxplot(chase.distance.m~encounter+time,data=chase.sample5.time,cex.axis=0.7)
####
...
lm.chase.phylo <- lm(CV~phylo,data=chase.phylo)
# add in quadratic
